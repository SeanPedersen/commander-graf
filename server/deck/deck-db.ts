...
export function initDeckSchema(db: Database.Database): void {
  db.exec(`
...
export class DeckStore {
  ...
  private prepareStatements() {
    return {
      ...
      getActiveAgents: this.db.prepare(
        `SELECT * FROM deck_agents WHERE status IN ('running', 'idle') ORDER BY started_at DESC`
      ),
      ...
      getCostByAgent: this.db.prepare(`
        SELECT agent_id, SUM(cost_usd) as total_cost, SUM(input_tokens) as total_input,
               SUM(output_tokens) as total_output
        FROM deck_cost_snapshots GROUP BY agent_id
      `),
      ...
      insertWorkflow: this.db.prepare(`
        INSERT INTO deck_workflows (id, name, config_json, status, total_cost, started_at)
        VALUES (?, ?, ?, ?, ?, ?)
      `),
  ...
  createAgent(config: SpawnAgentConfig): DeckAgent {
    const id = uuidv4();
  ...
  getAgent(id: string): DeckAgent | undefined {
    const row = this.stmts.getAgent.get(id) as any;
    if (row) {
      row.visible = true;
      row.interactive = !!row.interactive;
      row.session_id = row.session_id || null;
      row.runtime = row.runtime || "claude-code";
    }
    return row as DeckAgent | undefined;
  }

  private normalizeAgentRow(r: any): DeckAgent {
    return {
      ...r,
      visible: true,
  ...
  getAllAgents(): DeckAgent[] {
    const rows = this.stmts.getAllAgents.all() as any[];
    return rows.map((r) => this.normalizeAgentRow(r));
  }

  getActiveAgents(): DeckAgent[] {
    const rows = this.stmts.getActiveAgents.all() as any[];
    return rows.map((r) => this.normalizeAgentRow(r));
  }
  ...
  getAgentEvents(agentId: string, limit = 100): DeckEvent[] {
    return this.stmts.getAgentEvents.all(agentId, limit) as DeckEvent[];
  }

  getRecentEvents(limit = 50): (DeckEvent & { agent_name: string })[] {
    return this.stmts.getRecentEvents.all(limit) as (DeckEvent & {
      agent_name: string;
    })[];
  }
  ...
  addCostSnapshot(
    ...
    this.stmts.insertCostSnapshot.run(
      agentId,
      model,
      costUsd,
      inputTokens,
  ...
  getCostTimeSeries(): Array<{ hour: string; cost: number }> {
    return this.stmts.getCostTimeSeries.all() as Array<{
      hour: string;
      cost: number;
    }>;
  }
  ...
  saveSession(data: {
    ...
    this.stmts.insertSession.run(
      ...
      data.status,
      data.config_json,
      data.output_snapshot || null,
      null, // started_at set by trigger
  ...
  cleanupOrphans(): number {
    // Mark agents with stale PIDs as dead
